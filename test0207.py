...
for i in range(len(variables)):
    ttk.Label(window, text=variables[i]).pack()
    dropdown = ttk.Combobox(window, values=['none', '?tool', 'AI4PD:partOf', 'AI4PD:RapidMiner', '?toolbox', 'rdf:type', 'AI4PD:Toolbox', '?model', 'AI4PD:implementsMethod', '?method'], state='readonly')
    dropdown.current(0)
    dropdown.pack()
